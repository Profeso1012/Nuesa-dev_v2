...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({ 
      email, 
      password, 
      options: { data: { firstName, lastName, title } } 
    });
    setLoading(false);
    if (error) {
      alert(error.message);
    ...
    const userId = data?.user?.id || (data as any)?.id || null;
    if (userId) {
      try {
        await fetch('/api/admin/upsert-profile', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: userId, first_name: firstName, last_name: lastName, title })
        });
      } catch (e) {
        ...
        <div className="w-full max-w-sm lg:max-w-md mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex flex-col gap-2.5 p-2 px-5 rounded bg-[rgba(196,93,22,0.2)]">
              <label className="text-[#5B933C] font-bold text-xs tracking-wide">First name*</label>
              <input
            ...
            <div className="flex flex-col gap-2.5 p-2 px-5 rounded bg-[rgba(196,93,22,0.2)]">
              <label className="text-[#5B933C] font-bold text-xs tracking-wide">Last name*</label>
              <input
                className="bg-transparent text-[#212121] font-semibold outline-none"
                style={{ fontSize: '0.875rem', lineHeight: '1.25rem' }}
            ...
            <div className="flex flex-col gap-2.5 p-2 px-5 rounded bg-[rgba(196,93,22,0.2)]">
              <label className="text-[#5B933C] font-bold text-xs tracking-wide">Title*</label>
              <div className="flex items-center gap-2.5">
                <select
                  className="flex-1 bg-transparent text-[#212121] font-semibold outline-none appearance-none"
                  style={{ fontSize: '0.875rem', lineHeight: '1.25rem' }}
                  value={title}
                ...
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M18 9L12 15L6 9" stroke="#C93601" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
            </div>
            ...
            <div className="text-center text-sm pt-6">
              ...
              <a href="/login" className="text-[#E6731F] font-bold" style={{ fontSize: '0.875rem' }}>
                Login
              </a>
            </div>
          </form>
