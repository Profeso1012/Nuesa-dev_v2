...
function buildList(items: ExecutiveCard[]): ExecutiveCard[] {
  const filled = [...items];
  while (filled.length < 3) filled.push({ ...PLACEHOLDER, id: `__ph_${filled.length}` });
  return filled;
}

export const isPlaceholder = (item: ExecutiveCard) => item.id.startsWith('__');

function Card({ item, badgeColor }: { item: ExecutiveCard; badgeColor: string }) {
...
export default function ExecutiveCarousel({ items, badgeColor = 'bg-[#C93601]', autoInterval = 4000 }: Props) {
